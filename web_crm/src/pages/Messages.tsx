...
export default function MessagesPage() {
    ...
    const fetchAll = async () => {
        try {
            ...
            const allBookings: BookingSummary[] = Array.isArray(bookRes.data) ? bookRes.data : (bookRes.data.results || []);
            const directConvs: any[] = Array.isArray(convRes.data) ? convRes.data : (convRes.data.results || []);
            ...
            allBookings.forEach(b => {
                merged.push({
                    ...
                    subtitle: `Бронь #${b.id}`,
                    booking: b,
                    guestName: b.user_name,
                    timestamp: b.date
                });
            });

            // 2. Add Direct Conversations
            directConvs.forEach(c => {
                // If there's already a booking chat, maybe we don't need a "direct" one?
                // Actually, let's keep them separate for now, or merge if they share the same guest.
                // But a guest might have multiple bookings.

                // Let's only add it if it's not strictly tied to a recent booking
                // or just add it anyway as "General Chat".
                merged.push({
                    id: `c-${c.id}`,
                    type: 'direct',
                    targetId: c.id,
            ...
            if (!selectedConv && locationState?.bookingId) {
                const target = sorted.find(c => c.id === `b-${locationState.bookingId}`);
            ...
            <div className="w-80 shrink-0 flex flex-col border-r border-slate-200 bg-white">
                <div className="p-4 border-b border-slate-100">
                    <div className="flex items-center justify-between mb-4 px-1">
                        ...
                        <button onClick={fetchAll} className="p-1.5 flex items-center justify-center rounded-lg hover:bg-slate-100 text-slate-400 transition-colors">
                    ...
                    <div className="flex items-center gap-4 px-6 py-3 border-b border-slate-200 bg-white/50 backdrop-blur-sm">
                        ...
                        {selectedConv.type === 'booking' && selectedConv.booking && (
                            <>
                                ...
                                {isRestaurantRole && selectedConv.booking.status === 'pending' && (
